...
class Deque {
    struct Index {
        ...
        friend ptrdiff_t operator-(const Index& i_1, const Index& i_2) {
            auto i_1_general =
                static_cast<ptrdiff_t>(i_1.block_ * BLOCK_SIZE + i_1.index_);
            auto i_2_general =
                static_cast<ptrdiff_t>(i_2.block_ * BLOCK_SIZE + i_2.index_);
  ...
  public:
    ...
    class CommonIterator {
      public:
        ...
        friend auto operator<=>(const CommonIterator& it_1,
                                const CommonIterator& it_2) {
    ...
    Deque() : start_(), end_(), deque_cap_{1}, deque_(1) {}
    explicit Deque(size_t n)
        : start_(),
          end_(),
          deque_cap_{n / BLOCK_SIZE + static_cast<int>(n % BLOCK_SIZE > 0)},
          deque_(deque_cap_) {
        end_ += n;
        for (size_t i = start_.block_;
             i < end_.block_ + static_cast<int>(end_.index_ > 0); ++i) {
            deque_[i].MakeChunk();
        }
        Index cur = start_;
        try {
    ...
    Deque(const Deque& deque)
        : start_(deque.start_),
          end_(deque.end_),
          deque_cap_{deque.deque_cap_},
          deque_(deque.deque_.size()) {
        Index idx{start_};
        try {
            for (; idx != end_; ++idx) {
                if (idx.index_ == 0) {
    ...
    void erase(iterator it) {
        ...
        while (next != end_) {
            ...
            new (deque_[cur.block_].data() + cur.index_)
                T(deque_[next.block_][next.index_]);
            ++next;
            ++cur;
        }
        --end_;
    ...
    void insert(iterator it, const T& elem) {
        ...
        while (next != save) {
            ...
            new (deque_[next.block_].data() + next.index_)
                T(deque_[cur.block_][cur.index_]);
            --cur;
            --next;
        }
